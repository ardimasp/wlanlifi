function [packetErrorRate,varargout] = payload11aSim(...
    snr, cfgNHT, whichCase, SamplesPerSymbol, maxNumErrors, maxNumPackets)
%payload11aSim Simulates the 802.11a-based TGbb
%   A payload is defined as bits from L-SIG and PSDU
...
lstf = wlanLSTF(cfgNHT);
lltf = wlanLLTF(cfgNHT);
lsig = wlanLSIG(cfgNHT);
...
for isnr = 1:numSNR
    ...
    while numPacketErrors<=maxNumErrors && numPkt<=maxNumPackets
        ...
        data = wlanNonHTData(inpPSDU,cfgNHT);
        ...
        [rxLSIGBits, failCheck, eqLSIGSym] = wlanLSIGRecover(rx(pktOffset + (ind.LSIG(1):ind.LSIG(2)), :), ...
        ...
        txLSIGBits = wlanLSIGRecover(lsig,ones(52,1),0,cfgNHT.ChannelBandwidth);
