function [packetErrorRate,varargout] = payload11aSimPower(...
    ptx, cfgNHT, whichCase, SamplesPerSymbol, maxNumErrors, maxNumPackets)
%payload11aSim Simulates the 802.11a-based TGbb based on optical transmit power.
...
for iptx = 1:numPTX
    ...
    % Calculate packet error rate (PER) at Ptx point
    packetErrorRate(iptx) = numPacketErrors/(numPkt-1);
    disp(['MCS ' num2str(cfgNHT.MCS) ','...
          ' Ptx ' num2str(ptx(iptx)) ' W,'...
          ...
          ' PER:' num2str(packetErrorRate(iptx))]);
