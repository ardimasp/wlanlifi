function [packetErrorRate,varargout] = hesuSimFindFcWithCIR(...
    snr, cfgHE, centerFreq, whichCase, maxNumErrors, maxNumPackets, varargin)
...
tgbbChannel.awgnonlymode        = false;
...
if ~isempty(varargin)
    filename = ['hesuFindFcWithCIR_' 'MCS' num2str(cfgHE.MCS) '_fc' num2str(centerFreq/1e6) 'MHz' '_' varargin{1} '.mat'];
else
    filename = ['hesuFindFcWithCIR_' 'MCS' num2str(cfgHE.MCS) '_fc' num2str(centerFreq/1e6) 'MHz' '.mat'];
end
